...
func uploadFiles(srv hoclient.HostOrchestratorService, names []string, statePrinter *statePrinter) (string, error) {
	...
	uploadCompleted, err := srv.LockUploadDir(uploadDir)
	if err != nil {
		return "", fmt.Errorf("failed acquiring lock on upload dir: %w", err)
	}
	if !uploadCompleted {
		extractOps := []string{}
	...
	if err := srv.UnlockUploadDir(uploadDir); err != nil {
		return "", fmt.Errorf("failed releasing lock on upload dir: %w", err)
	}
