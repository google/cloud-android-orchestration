...
const (
	...
	userArtifactsDirName      = "/var/lib/cuttlefish-common/user_artifacts"
)
...
func (m *DockerInstanceManager) CreateHost(zone string, _ *apiv1.CreateHostRequest, user accounts.User) (*apiv1.Operation, error) {
	...
	volumeListRes, err := m.Client.VolumeList(ctx, volume.ListOptions{
		Filters: m.constructDockerFilter(user),
	})
	if err != nil {
	...
	if len(volumeListRes.Volumes) == 0 {
		_, err := m.Client.VolumeCreate(ctx, volume.CreateOptions{
			Name: m.getUAVolumeName(user),
			Labels: m.constructDockerLabel(user),
		})
		if err != nil {
		...
		Mounts: []mount.Mount{
			{
				...
				Source: m.getUAVolumeName(user),
				Target: userArtifactsDirName,
			},
		},
		Privileged: true,
	}
	...
	execConfig := types.ExecConfig{
		Cmd:          []string{"chmod", "777", userArtifactsDirName},
		AttachStdout: false,
		AttachStderr: false,
		Tty:          false,
	}
	...
	err = m.Client.ContainerExecStart(ctx, execRes.ID, container.ExecStartOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to change mode of user_artifacts diferctory: %w", err)
	}
...
func (m *DockerInstanceManager) getUAVolumeName(user accounts.User) string {
	return fmt.Sprintf("user_artifacts_%s", user.Username())
}

func (m *DockerInstanceManager) constructDockerFilter(user accounts.User) filters.Args {
...
func (m *DockerInstanceManager) constructDockerLabel(user accounts.User) map[string]string {
