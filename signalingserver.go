...
func (s *ForwardingSignalingServer) NewConnection(msg caoapi.NewConnMsg, user UserInfo) (*caoapi.SServerResponse, error) {
	dev, err := s.instanceManager.DeviceFromId(msg.DeviceId, user)
	if err != nil {
		return nil, err
	}
	var resErr ErrorMsg
	var reply caoapi.NewConnReply
	status, err := POSTRequest(hostURL(dev.Addr, "/polled_connections", ""), caoapi.NewConnMsg{dev.LocalId}, &reply, &resErr)
	if err != nil {
		return nil, err
	}
	if resErr.Error != "" {
		log.Println("The device host returned an error: ", resErr.Error)
		return &caoapi.SServerResponse{Response: resErr, StatusCode: status}, nil
	}
	...
	return &caoapi.SServerResponse{Response: reply, StatusCode: status}, nil
}

func (s *ForwardingSignalingServer) Forward(id string, msg caoapi.ForwardMsg, user UserInfo) (*caoapi.SServerResponse, error) {
	...
	return &caoapi.SServerResponse{reply, status}, nil
}

func (s *ForwardingSignalingServer) Messages(id string, start int, count int, user UserInfo) (*caoapi.SServerResponse, error) {
	dec, err := decodeConnId(id)
	if err != nil {
		return nil, NewNotFoundError("Invalid connection id", err)
	}
...
func (s *ForwardingSignalingServer) ServeDeviceFiles(params DeviceFilesRequest, user UserInfo) error {
	...
	if shouldIntercept(params.path) {
		...
		// It's safe to replace because the device id is generated by us and validated
		// on the call to DeviceFromId(), so it won't match the first part of the path
		params.r.URL.Path = strings.Replace(params.r.URL.Path, params.devId, dev.LocalId, 1)
