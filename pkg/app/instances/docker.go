...
func (m *DockerInstanceManager) downloadDockerImageIfNeeded(ctx context.Context) error {
	...
	out, err := m.Client.ImagePull(ctx, m.Config.Docker.DockerImageName, image.PullOptions{})
	if err != nil {
		return fmt.Errorf("Failed to pull docker image '%s': %w", m.Config.Docker.DockerImageName, err)
	}
	defer out.Close()
